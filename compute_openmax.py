...
def evaluate_osr_classifier(openmax_probs, labels, num_classes):
    tp = np.zeros(num_classes)
    fp = np.zeros(num_classes)
    fn = np.zeros(num_classes)

    for i in range(len(openmax_probs)):
        predicted_class = np.argmax(openmax_probs[i][:num_classes])
        true_class = labels[i]
    ...
    precision_ma, recall_ma = compute_precision_recall(tp.sum(), fp.sum(), fn.sum())
    precision_mi, recall_mi = compute_precision_recall(tp, fp, fn)

    f_measure_ma = compute_f_measure(precision_ma, recall_ma)
    f_measure_mi = compute_f_measure(precision_mi, recall_mi)

    return f_measure_ma, f_measure_mi

# Load the saved AVs and labels for test data
with open('avs_test_maxpool.pickle', 'rb') as f:
    avs_test = pickle.load(f)

# Load the true labels for test data
test_labels = np.array(testset.targets)

# Compute openmax probabilities for test data
openmax_probs_test = compute_openmax(mrs, mavs, avs_test, NUM_CLASSES)

# Evaluate OSR classifier for test data
f_measure_ma_test, f_measure_mi_test = evaluate_osr_classifier(openmax_probs_test, test_labels, NUM_CLASSES)
print("F-measure for test data - Macro Average:", f_measure_ma_test)
print("F-measure for test data - Micro Average:", f_measure_mi_test)